...
nearest_coords_in_range <- sf::st_is_within_distance(coords_sf, e_obs_lonlat_sf, dist = 10000, sparse = FALSE)

print("coords_sf -> ")
print(head(coords_sf))

print("Nearest -> ")

aha <- nearest_coords_in_range |> 
  tibble::as_tibble() |>
  dplyr::rowwise()  |>
  dplyr::summarise(n_in_range =  sum(c_across(where(is.logical))) )

coords_sf <- cbind(coords_sf, aha)

print("RESULT")
print(result[1:52,])


# print(nearest_coords_in_range)
