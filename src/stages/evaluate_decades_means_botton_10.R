  evaluate_decades_means_botton_10 <- function() {
  ...
  climatologies_mean_diffs_between_decades_bottom_10 <-
    ...
    dplyr::slice_min(diffs_between_decades, n = 10) |>
    dplyr::arrange(desc(diffs_between_decades))

  print(climatologies_mean_diffs_between_decades_bottom_10, n = 20)  
  ...
  dir.create(here::here(dirname(config$evaluate_decades_means_botton_10$out$graph_file_svg)), showWarnings = FALSE, recursive = TRUE)
  
  svglite::svglite(here::here(config$evaluate_decades_means_botton_10$out$graph_file_svg))
  plot(plot)
  invisible(
        dev.off()
  )

  # dir.create(here::here(dirname(config$evaluate_decades_means_botton_10$out$graph_dir_plotly)), showWarnings = FALSE, recursive = TRUE)

  # htmlwidgets::saveWidget(
  #   story_of_two_cities_july_plotly, 
  #   here::here(paste0(config$evaluate_decades_means_botton_10$out$graph_dir_plotly, "/index.html")),
  #   selfcontained = FALSE
  # )

  # saveRDS(story_of_two_cities_july_plotly, here::here(config$evaluate_decades_means_botton_10$out$graph_r_obj_plotly))  
