evaluate_2_cities_in_month_1 <- function() {
  # Configuration
  ...
  cities_in_study = config$evaluate_2_cities_in_month_1$the_2_cities
  month_in_study = config$evaluate_2_cities_in_month_1$month
  ...
  story_of_two_cities_july_base_plot <- climatologies_monthly_ext_dat |>
    ...
    ggplot2::labs(
      ...
      title = "Average Temperature in January - Tale of two Cities? or the same story..",
  ...
  dir.create(here::here(dirname(config$evaluate_2_cities_in_month_1$out$graph_file_svg)), showWarnings = FALSE, recursive = TRUE)
  
  svglite::svglite(here::here(config$evaluate_2_cities_in_month_1$out$graph_file_svg))
  ...
  # Save plotly graph
  # Create dir if it does not exist
  print(config$evaluate_2_cities_in_month_1$out$graph_dir_plotly)
  dir.create(here::here(config$evaluate_2_cities_in_month_1$out$graph_dir_plotly), showWarnings = FALSE, recursive = TRUE)
  
  htmlwidgets::saveWidget(
    story_of_two_cities_july_plotly, 
    here::here(paste0(config$evaluate_2_cities_in_month_1$out$graph_dir_plotly, "/index.html")),
    selfcontained = FALSE
  )

  saveRDS(story_of_two_cities_july_plotly, here::here(config$evaluate_2_cities_in_month_1$out$graph_r_obj_plotly))
